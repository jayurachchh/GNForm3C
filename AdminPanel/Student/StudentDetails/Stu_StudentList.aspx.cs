...
public partial class AdminPanel_Student_StudentDetails_Stu_StudentList : System.Web.UI.Page
{
    #region 11.0 Variables

    String FormName = "StudentDetails";
    ...
    private void Search(int PageNo)
    {
        ...
        if (!string.IsNullOrWhiteSpace(txtEnrollmentNo.Text.Trim()))
        ...
        if (!string.IsNullOrWhiteSpace(txtEmailInstitue.Text.Trim()))
            EmailInstitute = txtEmailInstitue.Text.Trim();

        if (!string.IsNullOrWhiteSpace(txtEmailPersonal.Text.Trim()))
        ...
        if (!string.IsNullOrWhiteSpace(txtRollNo.Text.Trim()))
        ...
        DataTable dt = stu_StudentDetailsBAL.SelectPage(Offset, PageRecordSize, out TotalRecords, StudentName, EnrollmentNo, CurrentSem, EmailInstitute, EmailPersonal, Gender, RollNo, ContactNo);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {
                    if (stu_StudentDetailsBAL.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        Stu_StudentDetailsBAL stu_StudentDetailsBAL = new Stu_StudentDetailsBAL();
        DataTable dt_student = stu_StudentDetailsBAL.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, StudentName, EnrollmentNo, CurrentSem, EmailInstitute, EmailPersonal, Gender, RollNo, ContactNo);
        if (dt_student != null && dt_student.Rows.Count > 0)
        {
            Session["ExportTable"] = dt_student;
    ...
    private void ClearControls()
    {
        ...
        txtEmailInstitue.Text = String.Empty;
